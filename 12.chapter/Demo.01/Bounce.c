...
/* Bounce - Creates a new thread eac time the letter 'a' is typed.
 * Each thread boundces a happy face of a different color around the screen.
 * All threads are terminated when the letter 'Q' is entered.
 * 
 * This propgram requires the multithread library. 
 * For example, compile with the following command line: 
 *		CL /MT BOUNCE.C
 */
...
void BounceProc(char* MyID)
{
	...
	do{
		...
		// If we still occupy the old screen position, black it out. 
